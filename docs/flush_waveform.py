# renders branch flush waveform svg
import csv
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt

rows = list(csv.DictReader(open('flush_wave.csv')))[3:19]
def col(n): return [int(r[n]) for r in rows]
pc = col('pc')
pcsrc = col('pc_src')
flush = col('flush')
x10, x5 = col('x10'), col('x5')
N = len(rows)

DEC = lambda v: f'{v}'
lanes = [('pc', 'bus', pc, DEC),
         ('pc_src', 'bit', pcsrc, None),
         ('flush', 'bit', flush, None),
         ('x10', 'bus', x10, DEC), ('x5', 'bus', x5, DEC)]
nlanes = len(lanes)
...
ax.set_title('Taken-Branch Flush', fontsize=13, pad=16)
plt.tight_layout()
plt.savefig('flush_waveform.svg', bbox_inches='tight', facecolor='white')
print('wrote flush_waveform.svg; rows', N)
