module embench_boot_tb ();
  ...
  initial begin
    rst = 1;
    if (!$value$plusargs("HEX=%s", hexfile)) hexfile = "sw/embench/crc32.hex";
    for (int k = 0; k < DEPTH; k++) img[k] = 32'h0;
    $readmemh(hexfile, img);
    if (img[0] == 32'h0) $fatal(1, "%s missing or empty, run make -C sw/embench all", hexfile);
    #1;  // After mem init
    ...
    $display("\n[embench_boot_tb] %s finished (LED sentinel 0xC0DE)", hexfile);
    $finish;
  end
